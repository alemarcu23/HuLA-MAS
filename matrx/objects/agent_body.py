...
# Diagonal movement is not supported in HuLA-MAS: agents may only move
# North/South/East/West. These move actions are excluded from the wildcard
# ("*") action set so no agent ever holds a diagonal move. Referenced by name
# (not imported) to avoid a circular import with matrx.actions.move_actions.
_DIAGONAL_MOVE_ACTIONS = {"MoveNorthEast", "MoveNorthWest",
...
class AgentBody(EnvObject):
    ...
    **custom_properties : dict, optional
    ...
    def __init__(self, location, possible_actions, sense_capability, class_callable,
        ...
        # Diagonal moves are excluded: only North/South/East/West movement is supported.
        if self.action_set == "*":
            self.action_set = [name for name
                               in _get_all_classes(Action, omit_super_class=True).keys()
                               if name not in _DIAGONAL_MOVE_ACTIONS]
