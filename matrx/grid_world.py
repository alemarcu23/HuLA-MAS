...
from matrx.messages.message_router import MessageRouter
...
from matrx.helpers.perception_helpers import _classify_type
...
class GridWorld:
    ...
    def __init__(self, shape, tick_duration, simulation_goal, rnd_seed=1,
        ...
        self.message_router = MessageRouter(self.message_manager)  # per-tick message routing for the game loop
    ...
    def __step(self):
        ...
        # Go over all agents. For each agent that is NOT mid-action, call its get_action_func
        # (-> filter_observations -> decide_on_action), store the returned action, mark the agent
        # busy for the action's duration, then move on to the next agent.
        # decide_on_action is expected to return immediately: an async agent (e.g. the LLM
        # Agent) submits its model call to a thread pool and returns Idle while the
        # future is pending, polling it on a later tick. The loop itself does not wait. If an
        # agent's decide_on_action blocks instead (e.g. a synchronous model call), the whole tick
        # stalls until it returns, freezing every other agent and the GUI.
        ...
        for agent_id, agent_obj in self.__registered_agents.items():
            ...
            # Collect this agent's outgoing messages (brain + API) and preprocess them.
            api_received = None
            if self.__run_matrx_api and agent_id in api._received_messages:
                api_received = api._received_messages[agent_id]
                del api._received_messages[agent_id]  # consumed for this tick
            self.message_router.collect_and_preprocess(
                self.__current_nr_ticks, agent_obj, all_agent_ids, self.__teams,
        ...
        # Group this tick's preprocessed messages by receiver.
        self.message_router.buffer_tick(self.__current_nr_ticks)
        ...
        # Deliver this tick's messages to their receivers.
        self.message_router.deliver(self.__registered_agents)
