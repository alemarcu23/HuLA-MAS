...
def get_latest_state_dicts(ids=None):
    """ In-process accessor for the latest stored states (no HTTP round-trip).

    Used by other servers running in the same Python process — notably the
    Human Supervisor view on the visualization server — to read MATRX state
    directly and build derived views (e.g. the cumulative "explored" map).

    Parameters
    ----------
    ids
        Optional iterable of agent IDs to fetch (e.g. ``['god']``). When None,
        the states of every stored agent (including ``'god'``) are returned.

    Returns
    -------
        Dict mapping ``agent_id`` -> plain (JSON-serializable) state dict, each
        containing a ``'World'`` entry. Empty dict if MATRX has not produced a
        tick yet, or the requested ids are not present.
    """
    if _gw_message_manager is None:
        return {}

    tick_states = __states.get(_current_tick)
    if not tick_states:
        return {}

    target_ids = list(tick_states.keys()) if ids is None else list(ids)

    out = {}
    for aid in target_ids:
        rec = tick_states.get(aid)
        if not rec:
            continue
        st = rec.get('state')
        if st is None:
