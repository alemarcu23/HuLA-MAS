...
class BaselineAgent(ArtificialBrain):
    def __init__(self, slowdown, condition, name, folder):
        ...
        self._human_names = []  # For multi-player: list of all human agent names
        self._human_locations = {}  # For multi-player: dict of human_name -> location
        self._folder = folder
    ...
    def decide_on_actions(self, state):
        # Identify team members and human agents (supports multi-player with 2 humans)
        agent_name = state[self.agent_id]['obj_id']
        for member in state['World']['team_members']:
            if member != agent_name and member not in self._team_members:
                self._team_members.append(member)
                # Check if this is a human agent and track it
                if member in state and 'is_human_agent' in state[member] and state[member]['is_human_agent']:
                    if member not in self._human_names:
                        self._human_names.append(member)

        # Update human locations for all human agents
        for member_id, member_info in state.items():
            if isinstance(member_info, dict) and 'is_human_agent' in member_info and member_info['is_human_agent']:
                if 'location' in member_info:
                    self._human_locations[member_id] = member_info['location']

        # Use first human for backwards compatibility with trust beliefs and single-player logic
        if len(self._human_names) > 0 and not self._human_names[0]:
            pass  # _human_name already set in __init__

        # Create a list of received messages from the human team members
