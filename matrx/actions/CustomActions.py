...
def _get_agent_capabilities(grid_world, agent_id):
    """Get capability dict from the agent BODY properties.

    Returns a permissive default (everything 'high') when no capabilities were
    attached, so agents outside the preset system (the human teammate, baseline
    bots) are never restricted — only configured AI agents are gated.
    """
    agent_body = grid_world.registered_agents[agent_id]
    return agent_body.properties.get('capabilities', {
        'vision': 'high', 'strength': 'high', 'medical': 'high', 'speed': 'normal'
    })
...
def _find_partner_agent(world_state, agent_id):
    ...
    for key, val in world_state.items():
        ...
        if not isinstance(inheritance, list):
            inheritance = []
        # Recognise any other agent (human or AI) as a possible partner.
        # MATRX flags every agent body with isAgent=True, and every agent
        # brain derives from AgentBrain. The previous check only matched the
        # LLM agent ('LlmAgent'), so cooperative actions silently
        # failed when the partner was the rule-based RescueBot (BaselineAgent)
        # or the human.
        if bool(val.get('isAgent')) or 'AgentBrain' in inheritance:
...
class CarryObject(Action):
    ...
    def is_possible(self, grid_world, agent_id, world_state, **kwargs):
        ...
        # Capability enforcement: medical skill controls which victims can be
        # carried solo. healthy -> anyone; mild -> medium/high; critical -> high.
        # Anything below the required level must use CarryObjectTogether.
        caps = _get_agent_capabilities(grid_world, agent_id)
        medical = caps.get('medical', 'high')

        if object_id and 'critical' in object_id and medical != 'high':
            return GrabObjectResult(
                f"Cannot carry critically injured victim '{object_id}' alone: it "
                f"requires high medical skill (yours is '{medical}'). Use "
                f"CarryObjectTogether with a partner instead.", False
            )
        if object_id and 'mild' in object_id and medical == 'low':
            return GrabObjectResult(
                f"Cannot carry mildly injured victim '{object_id}' alone: it "
                f"requires at least medium medical skill (yours is 'low'). Use "
                f"CarryObjectTogether with a partner instead.", False
            )
...
class Drop(Action):
    ...
    def is_possible(self, grid_world, agent_id, world_state, **kwargs):
        ...
        # Block solo drop ONLY while a cooperative carry is in progress (an
        # invisible partner is attached) — that victim must be released with
        # DropObjectTogether. A victim carried solo (any severity, e.g. a
        # critical carried by a high-medical agent) can always be dropped solo.
        if coop_carry_active and ('critical' in obj_id or 'mild' in obj_id):
            return DropObjectResult(
                "This victim is being carried cooperatively — use "
                "DropObjectTogether (not Drop) to release it.", False)
        else:
...
class CarryObjectTogether(Action):
    ...
    def is_possible(self, grid_world, agent_id, world_state, **kwargs):
        ...
        # Cooperative carry only requires both agents to be ADJACENT (within one
        # cell) of the victim — exactly like RemoveObjectTogether. The old rule
        # demanded both agents stand on the victim's *exact* tile, which is
        # physically impossible: two intraversable agents can never share a tile
        # (see move_actions.py), so a cooperative carry could never succeed.
        # world_state[object_id] is None when the object is already in another
        # agent's inventory — treat as not-in-range so the action fails gracefully
        # instead of crashing.
        coop_range = 1
        obj_loc = None
        if object_id:
            ...
            if get_distance(agent_loc, obj_loc) > coop_range:
                print("GrabObjectResult.NOT_IN_RANGE — agent not adjacent to victim.")
                return GrabObjectResult(GrabObjectResult.NOT_IN_RANGE, False)
        ...
        if partner is None:
            ...
            return GrabObjectResult(GrabObjectResult.NOT_IN_RANGE, False)
        if object_id and get_distance(tuple(partner['location']), obj_loc) > coop_range:
            print("GrabObjectResult.NOT_IN_RANGE — partner not adjacent to victim.")
            return GrabObjectResult(GrabObjectResult.NOT_IN_RANGE, False)
    ...
    def mutate(self, grid_world, agent_id, world_state, **kwargs):
        ...
        # 4. Victim now in agent's inventory — autopilot in LlmBrain
