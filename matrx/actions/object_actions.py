...
class RemoveObject(Action):
    ...
    def is_possible(self, grid_world, agent_id, **kwargs):
        ...
        # Capability enforcement: strength controls which obstacles can be removed
        # solo. tree -> anyone; stone -> medium/high; rock -> high. Anything below
        # the required level must use RemoveObjectTogether with a partner. This is
        # independent of vision/perception — only adjacency (above) matters.
        # A permissive default ('high') leaves the human / baseline bots unrestricted.
        caps = agent_avatar.properties.get('capabilities', {}) or {}
        strength = caps.get('strength', 'high')
        if 'rock' in object_id and strength != 'high':
            return RemoveObjectResult(
                f"Cannot remove big rock '{object_id}' alone: it requires high "
                f"strength (yours is '{strength}'). Use RemoveObjectTogether with "
                f"a partner instead.", False)
        if 'stone' in object_id and strength == 'low':
            return RemoveObjectResult(
                f"Cannot remove stone '{object_id}' alone: it requires at least "
                f"medium strength (yours is 'low'). Use RemoveObjectTogether with "
                f"a partner instead.", False)
