...
function ss_populate_agent_menu(state) {
    ...
    // preview of the supervisor view (oversees the team, explored-map only)
    var sup_preview = document.createElement("div");
    sup_preview.classList.add("agent_menu_preview");
    var sup_icon = new Image();
    sup_icon.src = window.location.origin + "/static/images/god.png";
    sup_preview.append(sup_icon);

    var sup_item = document.createElement('a');
    sup_item.classList.add('dropdown-item');
    sup_item.append(sup_preview);
    sup_item.appendChild(document.createTextNode('Supervisor'));
    sup_item.href = '/supervisor';
    sup_item.setAttribute('target', '_blank'); // open in a new tab
    dropdown.append(sup_item);
