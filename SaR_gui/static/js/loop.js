...
var lv_base_url = window.location.hostname,
    ...
    lv_agent_id = "",
    lv_session_id = "default_session",  // Session ID for multi-player
    lv_player_id = null,                 // Player ID (player1 or player2)
    lv_agent_type = null;
...
/*
 * Once the page has loaded, wait for lobby to complete, then call the initialization functions
 * NOTE: This runs AFTER the lobby initialization script, since it's loaded later in the HTML
 */
$(document).ready(function() {
    console.log("Loop.js ready, checking lobby status...");

    // Small delay to ensure lobby initialization has been called
    setTimeout(function() {
        console.log("Starting game initialization sequence...");

        // Wait for lobby to complete before starting the game loop
        var lobbyCheckInterval = null;
        var lobbyTimeout = null;
        ...
        if (typeof isGameStarted !== 'undefined') {
            console.log("Lobby system detected, waiting for lobby completion...");

            // Poll for lobby completion (max 15 seconds)
            lobbyCheckInterval = setInterval(function() {
                if (isGameStarted && isGameStarted()) {
                    console.log("Lobby completed, game started!");
                    startGameLoop();
                }
            }, 100);

            // Fallback: Start anyway after 15 seconds
            lobbyTimeout = setTimeout(function() {
                console.warn("Lobby timeout after 15 seconds, starting game anyway...");
                if (lobbyCheckInterval) clearInterval(lobbyCheckInterval);

                // Hide lobby, show game
                var lobbyContainer = document.getElementById('lobby-container');
                var gameContainer = document.getElementById('game-container');
                if (lobbyContainer) lobbyContainer.classList.add('d-none');
                if (gameContainer) gameContainer.classList.remove('d-none');

                // Try to get agent ID from lobby
                if (typeof getAgentId !== 'undefined') {
                    var agentId = getAgentId();
                    if (agentId) {
                        console.log("Using agent ID from lobby:", agentId);
                    }
                }
            ...
            console.log("Lobby system not available, starting game loop directly...");
            startGameLoop();
        }
    }, 500); // Wait 500ms for lobby to initialize
});
...
function world_manager_loop() {
    // CRITICAL FIX: Add delay to give MATRX time to transition from paused to running
    // After unpauseWorld() is called, MATRX needs ~500-1000ms to actually start responding
    setTimeout(function() {
        console.log("Game loop delayed initialization starting (1s delay for MATRX startup)...");
        init();
...
function initial_connect() {
    ...
    // Check if we have an agent_id from the lobby system
    if (typeof getAgentId !== 'undefined' && getAgentId()) {
        // Use the agent_id assigned by the lobby system (Human1 or Human2)
        lv_agent_id = getAgentId();
        console.log("Using agent ID from lobby system:", lv_agent_id);
    } else {
        // Fallback: Get the agent ID from the url (e.g. "god", "agent_0123", etc.)
        ...
        console.log("Using agent ID from URL:", lv_agent_id);
    }

    // CRITICAL: Validate agent ID is not empty
    if (!lv_agent_id || lv_agent_id === '' || lv_agent_id === 'session1' || lv_agent_id === 'default_session') {
        console.error("FATAL ERROR: Agent ID is invalid:", lv_agent_id);
        console.error("Lobby may not have initialized properly!");
        console.error("Expected agent_id like 'Human1' or 'Human2', got:", lv_agent_id);
        // Don't proceed - show error instead of crashing later
        if (typeof showLobbyError !== 'undefined') {
            showLobbyError('FATAL: Game did not initialize properly. Please reload the page.');
        }
        throw new Error('Invalid agent ID: ' + lv_agent_id);
    }

    // check if this view is for the god view, agent, or human-agent, and get the correct urls
    if (lv_agent_type == "" && lv_agent_id == "god") {
        lv_agent_type = "god";
...
//        console.log("Fps: ", lv_fps);
    ...
    // if we requested an update check if it was successful
        ...
        lv_update_request.done(function(data) {
            ...
            // SAFETY CHECK: Only draw if world_settings is properly initialized
            if (lv_world_settings && lv_world_settings['tick_duration']) {
                draw(lv_state, lv_world_settings, lv_messages, lv_chatrooms, new_tick = true);
            } else {
                console.warn("Waiting for world settings to initialize...");
            }
            request_new_frame();
        })
...
function get_MATRX_update() {
    ...
    // CRITICAL FIX: MATRX converts agent IDs to lowercase, so we need to query with lowercase
    // Browser uses "Human1"/"Human2" but MATRX has them as "human1"/"human2"
    var matrx_agent_id = lv_agent_id.toLowerCase();

    data = {"agent_id": matrx_agent_id, "chat_offsets": chat_offsets};
    ...
    var lv_update_request = $.ajax({
        ...
        success: function(data) {
            ...
            if (!data || !data['states'] || data['states'].length === 0) {
                console.error("MATRX API error: Invalid response structure");
            ...
            // CRITICAL FIX: MATRX stores agents in lowercase, so check for lowercase version
            var matrx_agent_id = lv_agent_id.toLowerCase();

            // Check if agent exists in the response
            if (!Object.keys(latestState).includes(matrx_agent_id)){
                console.error("MATRX API error: Agent not in response");
                console.error("Agent ID (uppercase from lobby):", lv_agent_id);
                console.error("Agent ID (lowercase for MATRX):", matrx_agent_id);
                console.error("Available agents:", Object.keys(latestState));
            ...
            // SAFETY CHECK: Validate agent state
            const agentData = latestState[matrx_agent_id];
            if (!agentData || !agentData['state']) {
                console.error("MATRX API error: Agent state missing");
            ...
            lv_state = agentData['state'];

            // SAFETY CHECK: Validate world data
            if (!lv_state['World']) {
                console.error("MATRX API error: World data missing");
                lv_first_tick = false;
                lv_reinitialize_vis = true;
                return;
            }
            ...
            // OPTIMIZED: More aggressive polling for faster updates across network
            // Reduced from 0.6 * tick_duration to 0.3 for faster response times
            lv_wait_for_next_tick = lv_tick_duration * 1000 * 0.3;

            // request at least every 100ms instead of 500ms for low-latency multiplayer
            if (lv_wait_for_next_tick > 100) {
                lv_wait_for_next_tick = 100;
            }
...
function send_userinput_to_MATRX(data) {
    ...
    // CRITICAL FIX: Use lowercase agent_id to match MATRX registration
    var matrx_agent_id = lv_agent_id.toLowerCase();
    var lv_resp = $.ajax({
        method: "POST",
        url: lv_send_userinput_url + matrx_agent_id,
        contentType: "application/json; charset=utf-8",
        dataType: 'json',
        data: JSON.stringify(data),
        success: function() {
