"""
main_multi_human.py — HuLA-MAS Search & Rescue with a human teammate.

Same LLM team as main.py, plus one keyboard-controlled human player (the HuLA
"human-led" setup). All tunable settings live in config.yaml; this runner only
adds the human (include_human=True) and starts paused until the player is ready.
"""
import os
import pathlib
import argparse
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="HuLA-MAS human-led Search & Rescue")
    parser.add_argument('--config', type=str, default='config.yaml',
                        help='Path to the YAML run config (default: config.yaml)')
    parser.add_argument('--session-id', type=str, default='default_session',
                        help='Session ID for multi-player coordination')
    parser.add_argument('--name', type=str, default='human',
                        help='Human player name (default: human)')
    ...
    print(f"Map: {map_name} | {num_agents} LLM agent(s) + 1 human ({args.name}) | "
          f"model={llm_cfg.get('model')}")

    log_dir = os.path.join(fld, 'logs', 'human_run')
    ...
    try:
        ...
        vis_thread = visualization_server.run_matrx_visualizer(
            verbose=False, media_folder=media_folder,
            session_id=args.session_id, multi_human=True)

        world = builder.get_world()
        print("Started world...")
        builder.api_info['matrx_paused'] = True  # wait until the player is ready
        ...
        if vis_thread is not None:
            try:
                requests.get(f"http://{flask_host}:{flask_port}/shutdown_visualizer", timeout=5)
